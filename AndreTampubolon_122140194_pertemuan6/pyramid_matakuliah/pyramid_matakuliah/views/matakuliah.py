...
def matakuliah_add(request):
    ...
    try:
        ...
        try:
            ...
            return HTTPBadRequest(json_body={'error': 'Field sks dan semester harus berupa angka'})
    ...
    try:
        ...
        if 'sks' in json_data:
            try:
                ...
                return HTTPBadRequest(json_body={'error': 'Field sks harus berupa angka'})
        if 'semester' in json_data:
            try:
                matakuliah.semester = int(json_data['semester'])
            except ValueError:
                return HTTPBadRequest(json_body={'error': 'Field semester harus berupa angka'})
        if 'deskripsi' in json_data:
            matakuliah.deskripsi = json_data['deskripsi']
